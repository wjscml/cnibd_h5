    ...
    <div class="columnist-author" v-wechat-title="authorInfo.nickname">
      ...
      <scroll @scroll="scroll" @touchToEnd="touchToEnd" :data="news" :pullUpLoad="pullUpLoad" :probeType="probeType" :listenScroll="listenScroll" class="list" ref="list">
        <div>
          ...
          <load-tips v-if="news.length == 0 || news.length > 4" :tips="tips" :isLoad="isLoad"></load-tips>
        </div>
      </scroll>
...
import { getColumnistAuthor, focusAuthor } from 'api'
...
export default {
  ...
  methods: {
    ...
    getAuthor () {
      getColumnistAuthor(this.params).then(res => {
        this.authorInfo = res.userInfo
        this.$nextTick(() => {
        ...
        share(this.shareVal)
        if (res.publishArticles.length !== 0) {
          this.news = res.publishArticles
        } else {
          this.tips = '暂未发表文章'
        }
      })
    ...
    loadMore () {
      ...
      getColumnistAuthor(moreParams).then(res => {
        if (res.publishArticles.length !== 0) {
          this.news = this.news.concat(res.publishArticles)
          this.tips = '上滑加载更多'
          this.isLoad = false
        } else {
          this.tips = '没有更多数据了~'
